...
| [GPU_Architecture_And_CUDA_Kernels](GPU_Architecture_And_CUDA_Kernels.ipynb) | SMs, warps, coalescing, bank conflicts, occupancy and tensor cores — driven by **fifteen real CUDA programs in [`kernels/`](kernels/)** that build from a memory copy to copy-on-write KV paging, scored end to end by the [`kernelbench`](kernelbench/) eval harness. Compiles with `nvcc` on a GPU, or with `g++` against a CPU shim that runs one thread per CUDA thread | CPU ✨ |
...
| [Agent_Workloads_On_The_Metal](Agent_Workloads_On_The_Metal.ipynb) | What an agent loop does to the metal: quadratic prefill, the **KV cache held hostage** through every tool call, cascade attention for fan-out, copy-on-write forking, and determinism as a cache-hit feature — compiling and running three real CUDA kernels | CPU ✨ |
