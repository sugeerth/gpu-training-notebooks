...
| [GPU_Architecture_And_CUDA_Kernels](GPU_Architecture_And_CUDA_Kernels.ipynb) | SMs, warps, coalescing, bank conflicts, occupancy and tensor cores — driven by **nineteen real CUDA programs in [`kernels/`](kernels/)** that build from a memory copy to the block-hash lookup that decides whether an agent's context needs prefilling at all, scored end to end by the [`kernelbench`](kernelbench/) eval harness. Compiles with `nvcc` on a GPU, or with `g++` against a CPU shim that runs one thread per CUDA thread | CPU ✨ |
...
| [Agent_Workloads_On_The_Metal](Agent_Workloads_On_The_Metal.ipynb) | What an agent loop does to the metal, in nine parts and **seven CUDA kernels it compiles and runs**: quadratic prefill and the block-hash lookup that stops it, cascade attention for fan-out, the **KV cache held hostage** through every tool call, ragged batches of mixed turn numbers, grammar masks and why speculation suits agents, copy-on-write forking, and **batch-invariant reductions** — the determinism a replay depends on | CPU ✨ |
